...
class MACDOnePercentShortOptimizedStrategy[T: BarBuilder](
  closePriceIndicator: OdinClosePriceIndicator[T],
  barCount: Int = 200
) extends BackTestStrategy {

  private val odinMACDOnePercentShortOptimizedStrategy = new RiskManagementStrategy(defaultStopLoss).andThen(
    new OdinMACDOnePercentShortOptimizedStrategy[T](closePriceIndicator, barCount)
  )
  ...
  def evaluate(
    ...
    odinMACDOnePercentShortOptimizedStrategy.evaluate(currentTrade, timestamp, currentPrice) match {
