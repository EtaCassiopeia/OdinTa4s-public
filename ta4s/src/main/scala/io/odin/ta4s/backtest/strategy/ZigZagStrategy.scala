...
import io.odin.ta4s.strategy.harmonic.patterns.OdinZigZagStrategy
import io.odin.ta4s.strategy.{ClosePosition, KeepCurrentSate, OpenPosition, RiskManagementStrategy, TakeProfitStrategy}

class ZigZagStrategy[T: BarBuilder](closePriceIndicator: OdinClosePriceIndicator[T], barCount: Int = 200)
  ...
  private val odinZigZagStrategy = new RiskManagementStrategy(defaultStopLoss)
    ...
    .andThen(
      new OdinZigZagStrategy[T](closePriceIndicator, barCount)
    )
