...
class MACDOnePercentStrategy[T: BarBuilder](closePriceIndicator: OdinClosePriceIndicator[T], barCount: Int = 200)
    extends BackTestStrategy {

  private val odinMACDOnePercentStrategy = new RiskManagementStrategy(defaultStopLoss).andThen(
    new OdinMACDOnePercentStrategy[T](closePriceIndicator, barCount)
  )
  ...
  def evaluate(
    ...
    odinMACDOnePercentStrategy.evaluate(currentTrade, timestamp, currentPrice) match {
